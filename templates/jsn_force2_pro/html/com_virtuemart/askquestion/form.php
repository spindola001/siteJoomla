				...
				<form method="post" class="form-validate" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$this->product->virtuemart_product_id.'&virtuemart_category_id='.$this->product->virtuemart_category_id.'&tmpl=component', FALSE) ; ?>" name="askform" id="askform">
					...
					<table class="askform">
						<tr>
							...
							<td><input type="text" class="validate[required,minSize[3],maxSize[64]]" placeholder="<?php echo $this->user->name ? $this->user->name : $sessData['name'] ?>" value="" name="name" id="name" size="30"  validation="required name"/></td>
						</tr>
						<tr>
							<td><label for="email"><?php echo vmText::_('COM_VIRTUEMART_USER_FORM_EMAIL')  ?> : </label></td>
							<td><input type="text" class="validate[required,custom[email]]" placeholder="<?php echo $this->user->email ? $this->user->email : $sessData['email'] ?>" value="" name="email" id="email" size="30"  validation="required email"/></td>
						</tr>
						<tr>
