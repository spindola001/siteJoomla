...
class SunFwSite
{
	...
	public function finalizeOutput()
	{
		...
		if (!empty($systemData['cdn_url']))
		{
			...
			// Find all asset links.
			if (preg_match_all('#("|\'|\()([^"\'\)\r\n]+)(\.[a-z0-9]{2,4})(\?[a-z0-9]+)?("|\'|\))#i', $buffer, $matches, PREG_SET_ORDER))
			{
				foreach ($matches as $match)
				{
					// Do not load HTML and PHP file from CDN.
					if (!in_array(strtolower($match[3]), array('.htm', '.html', '.php')))
					{
						...
						{
							$tmp_links[] = str_replace(JPATH_ROOT . '/', JUri::base(), $tmp_path);
							$tmp_links[] = str_replace(JPATH_ROOT, JUri::base(true), $tmp_path);

							if (preg_match('#^(' . implode('|', $tmp_links) . ')/#', $match[2]))
							{
								$ignore_cdn = true;
						...
						$cdn_url = preg_replace(
							array('#^' . JUri::base() . '#', '#^' . JUri::base(true) . '#'),
							array("{$systemData['cdn_url']}/", $systemData['cdn_url']),
							$match[2]
						);
						...
						if ($cdn_url != $match[2]);
						{
							$buffer = str_replace(
								$match[0],
								"{$match[1]}{$cdn_url}{$match[3]}{$match[4]}{$match[5]}",
								$buffer
							);
						}
					}
