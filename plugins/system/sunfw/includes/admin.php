...
class SunFwAdmin
{
	...
	public function __construct()
	{
		...
		// Get template data.
		$this->templateForm = JForm::getInstance(
            'com_templates.style',
            'style',
			array(
				'control' => 'jform',
	...
	public function renderHtml()
	{
		...
		<script type="text/javascript">
			// Add 'jsn-bootstrap4' class to the body tag.
			document.body.classList.add('jsn-bootstrap4');
		...
		foreach ($patterns as $pattern)
		{
			if (preg_match_all($pattern, $header, $matches, PREG_SET_ORDER))
			{
				foreach ($matches as $match)
				{
					if (!preg_match('#(/jquery\.js|/jquery\.min\.js|/plugins/system/sunfw/assets/)#', $match[1]))
					{
