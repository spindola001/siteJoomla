...
class SunFwHelper
{
	...
	/**
	 * Get list of module position for the specified template.
	 *
	 * @param   string  $template  Template name.
	 *
	 * @return  array
	 */
	public static function getModulePositions($template)
	{
	...
	/**
	 * Get default style for the specified template.
	 *
	 * @param   string  $template  Template name.
	 *
	 * @return  mixed
	...
	public static function getSunFwStyle($id, $force = false)
	{
		...
		if ($force || !isset($styles) || !array_key_exists($id, $styles))
		{
			$db = JFactory::getDbo();
			...
			try
			{
				if ($styles[$id] = $db->loadObject())
				{
					// Prepare appearance data for backward compatible.
					if ($appearance = json_decode($styles[$id]->appearance_data, true))
					{
						// Make sure color settings has only main and sub color.
						if (@is_array($appearance['general']) && @is_array($appearance['general']['color']))
						{
							foreach ($appearance['general']['color'] as $k => $v)
							{
								if (!in_array($k, array('main-color', 'sub-color')))
								{
									unset($appearance['general']['color'][$k]);
								}
							}
						}

						$styles[$id]->appearance_data = json_encode($appearance);
					}
				}
				else
				{
					// Get template name.
					$template = $db->setQuery(
						$db->getQuery(true)
							->select('template')
					...
					if (!empty($template))
					{
						// Check if the template has a default structure.
						$xml = SunFwHelper::getManifest($template);

						if (empty($defaultStructure = (string) $xml->defaultStructure))
						{
							$structures = glob(JPATH_THEMES . "/{$template}/structures/*.json");

							if (count($structures) == 1)
							{
								$structures = explode(DIRECTORY_SEPARATOR, current($structures));
								$defaultStructure = substr(array_pop($structures), 0, -5);
							}
						}
						...
						if ($defaultStructure)
						{
							...
							include_once SUNFW_PATH . '/libraries/3rd-party/scssphp/scss.inc.php';

							// Install the default structure.
							JFactory::getApplication()->input->set('structure_id', $defaultStructure);

							$installer = new SunFwAjaxSampleData($template, $id);

							$installer->installStructureAction();

							// Re-get style data.
							return self::getSunFwStyle($id);
						}
					}
				}
			}
	...
	public static function getVersionData()
	{
		...
		{
			self::$versionData = SunFwHttp::get(SUNFW_VERSIONING_URL . '?category=cat_template');
		}
	...
	public static function getSampleDataList($template)
	{
		...
		if (!isset($samples) || !isset($samples[$template->id]))
		{
			...
			$samples[$template->id] = SunFwHttp::get(
				"https://www.joomlashine.com/sunfwdata/sampledata/templates/{$template->id}.json",
				3600
			);
		}
	...
	public static function getActiveNicheStyle()
	{
		...
		{
			// Get active template.
			$template = PlgSystemSunFw::$template ?: JFactory::getApplication()->getTemplate(true);
	...
	/**
	 * Load all assets required by Sun Framework.
	 *
	 * @param   boolean  $admin      Whether to load assets for the template admin screen?
	 * @param   boolean  $bootstrap  Whether to load Bootstrap stand-alone?
	 * @param   string   $inline     Whether to load all neccessary assets inline?
	 *
	 * @return  string
	 */
	public static function loadAssets($admin = true, $bootstrap = false, $inline = false)
	{
		...
		{
			// Load Bootstrap.
			$doc->addStylesheet("{$base}/3rd-party/bootstrap/bootstrap.min.css");
			$doc->addStylesheet("{$base}/joomlashine/admin/css/modal.css?{$vd}");

			// If loading assets for template admin, load Bootstrap scripts also.
			if ($admin)
			{
				$doc->addScript("{$base}/3rd-party/popper/popper.min.js");
				$doc->addScript("{$base}/3rd-party/bootstrap/bootstrap.min.js");
			}

			// Load Font Awesome.
			$doc->addStylesheet("{$base}/3rd-party/font-awesome/css/font-awesome.min.css");
		}

		if ($admin)
		{
			// Load Template Admin stylesheet.
			$doc->addStylesheet("{$base}/joomlashine/admin/css/style.css?{$vd}");

			// Load Noty.
			$doc->addStylesheet("{$base}/3rd-party/noty/animate.css");
			$doc->addScript("{$base}/3rd-party/noty/jquery.noty.js");

			// Load Interact.
			$doc->addScript("{$base}/3rd-party/interact/interact.min.js");
		}
		...
		$doc->addScript("{$base}/joomlashine/admin/js/helpers.js?{$vd}");
		$doc->addScript("{$base}/joomlashine/admin/js/sunfw.js?{$vd}");

		// Render document head and return if loading assets inline is necessary.
		if ($inline)
		{
	 ...
	 * @return  string
	 */
	public static function renderFeedbackModal($template, $inline = false)
	{
		// Define base Ajax URL.
		$ajaxBase = 'index.php?option=com_ajax&format=json&group=system&plugin=sunfw';
		$ajaxBase .= "&context=feedback&action=get&template_name={$template}";
		$ajaxBase .= '&' . JSession::getFormToken() . '=1';
		$ajaxBase = JRoute::_($ajaxBase, false);

		// Generate HTML code for rendering feedback modal.
		$html = "<div id='sunfw-uninstall-feedback' data-render='ComponentFeedback' data-url='{$ajaxBase}'></div>";

		// Load necessary assets.
		$html .= self::loadAssets(false, $inline ? true : false, $inline);
