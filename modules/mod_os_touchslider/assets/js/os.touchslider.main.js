...
(function () {
  ...
  var osSliderSettings = function (container, params) {
    ...
    oss.resetSlider = function (reinit){
      ...
      jQuerOSS(container + " .image-link-block .image-link-input").each(function(index, el){
        imageLink[jQuerOSS(el).parent().attr("data-image-id")] = jQuerOSS(el).val();
      });
    ...
    oss.addNewText= function (el){
      ...
      if(jQuerOSS(container + " .editing-text:visible").length){
        ...
        jQuerOSS(container+" .add-image-text,"+container+" .paste-image-text,"+
    ...
    oss.cancelTextEditor = function (el){
      ...
      if(jQuerOSS(el).length){
        jQuerOSS(container + " .existing-text,"+container +"  .image-time-block,"+container +"  .image-link-block,"
    ...
    oss.saveTextEditor = function (el){
      ...
      jQuerOSS(container+" .add-image-text,"+container+" .paste-image-text,"+container+" .back-image-edit,"+
    ...
    oss.pasteText = function(){
      if(oss.params.debugMode){
            ...
            if(jQuerOSS(SelectorForTextElementForPast).length > 1 )
              displayText = jQuerOSS(SelectorForTextElementForPast).first().clone(); //this need for fix loop option
            else displayText = jQuerOSS(SelectorForTextElementForPast).clone();
        ...
        function (responseJSON) {      
          ...
          if(responseJSON.img_id == false){
            ...
            var imageLinkInput = '<span data-image-id="'+img_id+'" class="image-link" style="display:none;">'+
                         ...
                         'class="fa fa-info-circle info_block"></i>'+
                         '<input class="image-link-input" type="text" name="image-link['+img_id+']" size="20" '+
                         ' maxlength="300" value="">'+
                        '</span>';
            jQuerOSS(container + " .image-link-block").append(imageLinkInput);
            ...
            //image background
              ...
              var background = '<span data-image-id="'+img_id+'" class="image-background" style="display:none;">Background:'+
                ...
                'class="fa fa-info-circle info_block"></i><input '+
            ...
            jQuerOSS.post("index.php?option=com_ajax&module=os_touchslider&Itemid="+oss.params.ItemId+"&moduleId="+oss.params.moduleId+"&format=raw&image_id="+parent_img_id+"&method=copyImage",
                      ...
                      if(oss.params.debugMode){
                        ...
                        var timeInput = '<span data-image-id="'+image_id+'" class="image-full-time" style="display:none;">'+
                                     ...
                                     ' value="'+jQuerOSS(".image-full-time[data-image-id="+parent_img_id+"] input").val()+'">'+
                                    '</span>';
                        jQuerOSS(container + " .image-time-block").append(timeInput);
                        ...
                        var imageLinkInput = '<span data-image-id="'+image_id+'" class="image-link" style="display:none;">'+
                                     ...
                                     ' maxlength="300" '+
                                     'value="'+jQuerOSS(".image-link[data-image-id="+parent_img_id+"] input").val()+'">'+
                                    '</span>';
                        jQuerOSS(container + " .image-link-block").append(imageLinkInput);
  ...
  oss.imageEdit = function (el){
    ...
    jQuerOSS(container +"  .image-link-block span[data-image-id='"+jQuerOSS(el).attr("data-image-id")+"']").show();
...
//   //end
  ...
  oss.saveSliderSettings = function (moduleId,copyimage){
    ...
    params.imageLink = jQuerOSS.toJSON(params.imageLink);
          ...
          onComplete: function (id, filename, responseJSON) {
            ...
            if (!responseJSON.success) {
              ...
              if(responseJSON.image_copy_id){
                ...
                imageLinkInput = jQuerOSS(container + " .image-link[data-image-id="+image_id+"]");
